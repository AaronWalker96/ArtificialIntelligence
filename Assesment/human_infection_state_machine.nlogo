...
; * GLOBALS *
; ***********
patches-own [ state  infected_time  treatment_timer revive_timer ]
globals [ targetX targetY ]
...
to set-up
  clear-all
  reset-ticks
  ask patches [ spawn-human ]
end

to setup-globals
  set targetX 0
  set targetY 0
  set treatment_timer treatment_time
end


to go
  ask patches
  [ ; healthy patches/humans are left alone
    ...
    if state = "treated"
    [ ifelse treatment_timer > 0
      [ set treatment_timer (treatment_timer - 1)]
      ; Determine if the treatment was successful
      [ ifelse (random 101) > treatment_succes_rate
        [ kill-human ]                      ; treatment unsuccessful
        [ spawn-human ]                     ; treatment successful - sadly a human can
      ]                                     ; get infected again right after he heals
    ]
...
to infect-human                         ; infects a specified human
  ask patch targetX targetY
  [ set state "infected"
    set pcolor 15
  ]
end

to apply-treatment                      ; applies treatment to a specified human
  ask patch targetX targetY
  [ if state = "infected"
    [ ask patch targetX targetY
      [ set state "treated"
        set pcolor 45
        set treatment_timer treatment_timer + infected_time
      ]
    ]
  ]
end
...
134
Mouse coords
if mouse-down?\n[ set targetX (round mouse-xcor )\n  set targetY (round mouse-ycor )\n ]
T
1
T
OBSERVER
...
895
292
Time left before a new healthy human spawns
12
0.0
1
