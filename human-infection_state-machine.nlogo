...
to spread-infection
  ask patches
  [ if state = "infected"
    [ if random 101 < infection_spread_rate      ; Based on infection aggressivity
      [ let legacy_gen genome
        ...
        [ set legacy_gen change-random-bit-n-times genome (1 + random (32 - 1)) ]
         ...
         [ ifelse state = "healthy"            ; Spread infection on a healthy idividual
           ...
           [ if state = "treated"              ; If individual was treated but treatment isn't effective to the attacking infection genome
